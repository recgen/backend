...
from health_backend.presentation.web_api.v2.error_handlers import (
    application_error_handler,
    domain_error_handler,
    infrastructure_error_handler,
)
from health_backend.presentation.web_api.v2.routes import auth, patient, recommendation


def include_routers(app: FastAPI) -> None:
    app.include_router(recommendation.router, prefix='/api/v2')
    app.include_router(auth.router, prefix='/api/v2')
    app.include_router(patient.router, prefix='/api/v2')
